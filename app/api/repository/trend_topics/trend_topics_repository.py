...
class TrendTopicRepository:
    ...
    async def find_today_trend_topics(self, today_date: datetime):
        trend_topics_in_collection = []
        date = today_date
        
        while not trend_topics_in_collection:
            cursor = self.__collection.find({"date": str(date)}).sort([
                ("date", 1),
                ("order", 1)
            ])
